...
KP_MODEL = str(SCRIPT_DIR / "models/rtmpose_s_with_post_rrect_openvino_2022.1_6shave.blob")
SCRIPT_DETECTION_TO_KP = str(
    SCRIPT_DIR / "pipeline_scripts" / "rtmpose_rr_post_inference" / "bbox_to_2d_kps_manager.py")


# SCRIPT_HAND_LM_CONF = str(SCRIPT_DIR / "pipeline_scripts" / "rtmpose_rr_post_inference" / "hand_lm_manager.py")
...
class TGTTracker:
    ...
    def __init__(
        ...
        self.pd_input_height, self.pd_input_width = 256, 192
        self.body_input_height, self.body_input_width = 320, 544
        ...
        assert input_src not in get_args(DeviceModel), f"Invalid input source: {input_src}"
        ...
        self.internal_fps = internal_fps or 20 - (2 * max_bodies)
        ...
        if input_src in get_args(TOF):
            self.internal_fps = min(15, self.internal_fps)
    ...
    def create_pipeline(self):
        ...
        kp_man_script = self.build_manager_script(SCRIPT_DETECTION_TO_KP, pipeline)
        ...
        kp_man_script.outputs["early_out_pd"].link(results_script.inputs["early_out_pd"])
        ...
        if self.xyz:
            ...
            if self.input_device in get_args(DisparityDepth):
                ...
                cam_b_rgb: n.Camera = pipeline.create(n.Camera)
                cam_b_rgb.setBoardSocket(CameraBoardSocket.CAM_B)
                cam = cam_b_rgb
            else:
        ...
        print("Creating Body Detection pre processing image manip...")
        pre_body_det_manip = pipeline.create(n.ImageManip)

        pre_body_det_manip.setMaxOutputFrameSize(
            self.body_input_width * self.body_input_height * 3
        )
        pre_body_det_manip.inputImage.setQueueSize(1)
        pre_body_det_manip.inputImage.setBlocking(False)

        pre_body_det_manip.initialConfig.setResize(self.body_input_width, self.body_input_height)
        pre_body_det_manip.initialConfig.setFrameType(ImgFrame.Type.RGB888p)

        cam.preview.link(pre_body_det_manip.inputImage)

        # Define landmark model
        print("Creating Body Detection Neural Network...")
        det_nn = pipeline.create(n.NeuralNetwork)
        det_nn.setBlobPath(blobconverter.from_zoo(name="person-detection-retail-0013", shaves=6))
        # det_nn.setNumInferenceThreads(2)
        pre_body_det_manip.out.link(det_nn.input)
        det_nn.out.link(kp_man_script.inputs["detection_results"])
        if self.trace & 4:
            det_nn.passthrough.link(kp_man_script.inputs["body_nn_frame"])
            debug_out_pose_link = pipeline.createXLinkOut()
            debug_out_pose_link.setStreamName("pd_manager_out")
            kp_man_script.outputs['pose_trace4_output'].link(debug_out_pose_link.input)
        ...
        pre_pose_manip = pipeline.create(n.ImageManip)
        pre_pose_manip.setMaxOutputFrameSize(
            self.pd_input_width * self.pd_input_height * 3
        )
        pre_pose_manip.setWaitForConfigInput(True)
        pre_pose_manip.inputImage.setQueueSize(1)
        pre_pose_manip.inputImage.setBlocking(False)
        cam.preview.link(pre_pose_manip.inputImage)
        kp_man_script.outputs["pre_pose_manip_cfg"].link(pre_pose_manip.inputConfig)

        # Define pose detection model
        print("Creating Pose Detection Neural Network...")
        pose_nn = pipeline.create(n.NeuralNetwork)
        pose_nn.setBlobPath(Path(self.kp_model))
        pre_pose_manip.out.link(pose_nn.inputs['kps_input_0'])
        kp_man_script.outputs["post_center_size_angles"].link(pose_nn.inputs['post_center_size_angles'])
    ...
    def build_manager_script(self, script_path, pipeline):
        ...
        with open(script_path, "r") as file:
            ...
            subs = {
                ...
                "_max_bodies": self.max_bodies,
                "_pad_h": self.pad_h,
                "_img_h": self.img_h,
                "_img_w": self.img_w,
                "_frame_size": self.frame_size,
                "_crop_w": self.crop_w,
                "_bbox_confidence_thresh": self.body_score_thresh,
                "_body_input_length": self.body_input_height,
            }
        ...
        if self.trace & 4:
            ...
            if pre_pd_manip:
                ...
                for idx, box in enumerate(pd_results.get("body_boxes", [])):
                    ...
                    cv2.putText(video_frame, f"p{idx}:{round(box[4], 2)}", (box[0], box[1]), cv2.FONT_HERSHEY_SIMPLEX,
                                1,
                                color)
